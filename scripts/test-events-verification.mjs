...
page.addInitScript(() => {
  ...
  Object.defineProperty(window, 'fbq', {
    get() { return original.fn; },
    set(fn) {
      original.fn = function (...args) {
...
page.on('request', (req) => {
  ...
  if (url.includes('/api/meta-capi')) {
    try {
      ...
      evidence.capiRequests.push({ url, body });
    } catch { evidence.capiRequests.push({ url, body: req.postData() }); }
  }
...
try {
  ...
  // Fill name + phone to trigger InitiateCheckout
  ...
  evidence.orderResponse = { status: orderResp.status() };
  try { evidence.orderResponse.json = await orderResp.json(); } catch {}
